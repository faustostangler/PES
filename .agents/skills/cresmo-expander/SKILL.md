...
All prose generated by this skill must strictly adhere to the authorial voice and standards codified in the [`cresmo-style-guide`](file:///home/stangler/gamer_d/Fausto%20Stangler/Documentos/Python/PES/.agents/skills/cresmo-style-guide/SKILL.md).
...
7. **Mandatory Adherence to Cresmo Style Guide**:
   ...
   - **Methodological Caution**: Signal epistemic status clearly when transitioning across analytical registers (consolidated evidence, plausible inference, original hypothesis).
   - **Zero Em-Dashes (`—`)**: Strictly NEVER EVER use em-dashes or long dashes anywhere in the prose. Use commas, parenthetical phrasing, or distinct sentences.
   - **Zero Antitheses**: Strictly NEVER EVER use binary contrast formulas or negative antitheses (e.g., "não X, mas Y", "não se trata de X, e sim de Y"). Assert every point affirmatively and directly.
...
- **Punctuation & Syntax Constraints**: Zero em-dashes (`—`) and zero binary antitheses. All statements must be affirmatively structured.
