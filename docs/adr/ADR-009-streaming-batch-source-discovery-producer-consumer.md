...
Priority sources (local markdown/text compendiums in `priority_texts_dir` and videos in `playlist_priority_path`) are resolved immediately from disk and yielded to the consumer in the first milliseconds of execution.

### 2.2 Background Crawler Producer Thread
If `enable_channel_crawler=True` and `media_ingestion_port` is available:
- Channel feed resolution and upload probing run in a dedicated background worker thread (`_CrawlerProducerThread`).
...
### 2.3 Streaming Generator & Dual Interface
